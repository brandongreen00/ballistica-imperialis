...
export const ATTACKER_EFFECTS = {
  ...
  from_darkness_death: {
    ...
    label: "From Darkness, Death (1 normal → crit)",
    type: "normal_to_crit_attack",
    params: { count: 1 },
    note: "Hand of the Archon strategy ploy — active operative selects one enemy operative it cannot target; first shoot/fight action against that enemy this activation, one retained normal hit becomes a critical hit",
  },
  prey_on_the_wounded: {
    id: "prey_on_the_wounded",
    label: "Prey on the Wounded (Relentless)",
    type: "add_rules",
    params: { rules: [{ name: "Relentless" }] },
    note: "Hand of the Archon firefight ploy — re-roll any attack dice when shooting/fighting a wounded enemy operative (modelled as Relentless)",
  },
  blade_artists: {
    id: "blade_artists",
    label: "Blade Artists (Rending)",
    type: "add_rules",
    params: { rules: [{ name: "Rending" }] },
    note: "Hand of the Archon strategy ploy — friendly operatives' melee weapons gain Rending; only enable when using a melee weapon",
  },
  toxin_coating: {
    id: "toxin_coating",
    label: "Toxin Coating (Lethal 5+)",
    type: "add_rules",
    params: { rules: [{ name: "Lethal", value: 5 }] },
    note: "Hand of the Archon equipment — up to twice per turning point, melee weapons gain Lethal 5+ until end of fight/retaliation sequence; only enable when using a melee weapon",
  },
  refined_poison: {
    id: "refined_poison",
    label: "Refined Poison (+1 Normal Dmg)",
    type: "modify_dmg",
    params: { normal: 1 },
    note: "Hand of the Archon equipment — up to twice per turning point, splinter weapons (splinter rifle, splinter pistol, splinter cannon, stim-needler, stinger pistol) gain +1 Normal Damage during Shoot actions; don't enable with non-splinter weapons",
  },
  wicked_blades: {
    id: "wicked_blades",
    label: "Wicked Blades (+1 Atk)",
    type: "modify_atk_dice",
    params: { amount: 1 },
    note: "Hand of the Archon equipment — adds 1 to the Atk stat of array of blades; only enable when using that weapon",
  },
  hidden_engagement: {
